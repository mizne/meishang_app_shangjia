...
@Component({
  selector: 'app-store-certification-success',
  templateUrl: './store-certification-success.page.html',
  styleUrls: ['./store-certification-success.page.scss'],
})
export class StoreCertificationSuccessPage implements OnInit {
  TenantId: string;
  UserId: string;
  ExhibitorId: string;
  RecordId: string;
  ...
  // 获取营业证件
  // 此页面为审核通过页，审核通过时无法修改，所以不需要更改上传接口，注意！！！
  // 此页面为审核通过页，审核通过时无法修改，所以不需要更改上传接口，注意！！！
  // 此页面为审核通过页，审核通过时无法修改，所以不需要更改上传接口，注意！！！
  busi() {
    if (this.state === '2') {
  ...
  save() {
    ...
    var reg1 = (/^\d{15}$/).test(li);
    if (reg === false || reg1 === false) {
